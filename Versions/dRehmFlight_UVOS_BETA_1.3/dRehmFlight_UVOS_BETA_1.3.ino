...
//REQUIRED LIBRARIES (included as part of UVOS library)
#include <uvos.h>
#include "uvos_brd.h"
...
/* Channel convention used in code.
Channel Number|          |      |
  (In Code)   | Function | IBus | Notes
--------------|----------|------|---------------------------------------------
     1        | Throttle |  2   | 1000 min, 2000 max
     2        | Roll     |  0   | 1000 full left, 2000 full right, 1500 center
     3        | Pitch    |  1   | 1000 pitch up, 2000 pitch down, 1500 center
     4        | Yaw      |  3   | 2000 full left, 1000 full right, 1500 center
     5        | Gear     |  6   | Throttle cut when greater than 1500
     6        | Aux1     |  9   | Free auxiliary channel
*/

/* RC receiver API uses channel 1 for first channel, so first val is dummy value */
const int8_t  RC_MAP[SERRX_NUM_CHAN+1] = {-1,2,0,1,3,4,7,5,6,8,9};
...
void getCommands() {
  ...
  channel_2_pwm = getRadioPWM(RC_MAP[2]);
  channel_3_pwm = getRadioPWM(RC_MAP[3]);
  channel_4_pwm = getRadioPWM(RC_MAP[4]);
  channel_5_pwm = getRadioPWM(RC_MAP[5]);
  channel_6_pwm = getRadioPWM(RC_MAP[6]);
